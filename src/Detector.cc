...
Detector::Detector()
{
    ...
    // net = cv::dnn::readNetFromONNX("cfg/yolov5n-seg.onnx");
    ...
    // Code for CSE-598
    cout << "loading yolo v5 network...";   
    if (yolov5->ReadModel("cfg/yolov5n-seg.onnx", false)) {
        cout <<  "done\n";
    } else {
        cout << "FAILED\n";
    }
...
void Detector::Detect(){
        ...
        if (yolov5->OnnxDetect(mImg, result)) {
        ...
        for (size_t i = 0; i < result.size(); ++i){
			if(yolov5->_className[result[i].id] == "person"){
				people_counter++;
				people_boxes.push_back(result[i].box);
                people_masks.push_back(result[i].boxMask);

				aprox_area = aprox_area + (result[i].box.width * result[i].box.height)/fig_area;				
			}
		}
		//
