...
// 		}
		...
		std::cout << "SetGraphOptimizationLevel" << std::endl;

// #ifdef _WIN32
// 		std::cout << "It's _WIN32" << std::endl;
...
// #else
		std::cout << "ORT Session starting..." << std::endl;
		_OrtSession = new Ort::Session(_OrtEnv, modelPath.c_str(), _OrtSessionOptions);
		std::cout << "ORT Session started" << std::endl;
// #endif

		
		//init input
		_inputNodesNum = _OrtSession->GetInputCount();
		std::cout << "ORT Session input count read" << _inputNodesNum << std::endl;
